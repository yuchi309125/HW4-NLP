...
def create_augmented_dataloader(args, dataset):
    ################################
    ##### YOUR CODE BEGINGS HERE ###
    # 1. 取原本 IMDB 的 train split
    train_dataset = dataset["train"]

    # 2. 從 train 隨機抽 5000 筆
    sampled = train_dataset.shuffle(seed=42).select(range(5000))

    # 3. 對這 5000 筆做 transformation（synonym + typo）
    transformed = sampled.map(custom_transform, load_from_cache_file=False)

    # 4. tokenize transformed data
    ...
    # 5. 把 transformed + 原本的 tokenized train 合起來
    augmented_dataset = datasets.concatenate_datasets(
        [transformed_tokenized, tokenized_dataset["train"]]
    )
